...
/*export*/ class Component {
	...
	_render(opts=EMPTY) {
		...
		//transformHyperscript(rendered);
		//this.base = build(this.base, rendered);
		...
		//console.log(`${this.constructor.name}.render(): `, rendered, this.base);

		// if (this.base && this.__handlers) {
		// 	this._handlers.forEach( v => this.base.removeEventListener(...v) );
		// }
		//this.__handlers = [];
		//transformHyperscript(rendered, this.__handlers, this.__key);

		//transformHyperscript(rendered);
		//this.base = createControlsFromJSON(rendered);
	...
	if (args.length) {
		...
		if (typeof content==='string') {
			...
			if (Array.isArray(content)) {
				if (options.collapseTextNodes===true && content.length===1 && typeof content[0]==='string') {
					text = content[0];
				}
				children = content;
			}
...
// 				children[i] = new VNode('x-text', null, children[i]);
// 			}
// 		}
// 	}
...
function build(dom, vnode) {
	...
	// let children = out.childNodes;
	// let keys = vnode.children.map( child => child.attributes.key );
	...
	// 		}
	// 		if (ind!==-1) {
	// 			out.insertBefore(child, children[ind]);
	// 		}
	// 	}
	// }
	...
	if (vnode.children) {
		for (let i=0, vlen=vnode.children.length; i<vlen; i++) {
			...
			// failed to find anything similar, just create the new node
			//if (!child) {
			//	child = recycler.create(vchild.nodeName);
			//}
...
// function transformHyperscript(vnode, key='0') {
// 	// support for composing components:
...
// 	}
//
// 	// let attrs = root.attributes || (root.attributes = {});
// 	// attrs['component-id'] = key;
// 	// if (attrs) {
// 	// 	for (let p in attrs) {
// 	// 		if (p.match(/^on[a-z]/i)) {
// 	// 			handlers.push([ p.substring(2), `[component-id="${key}"]`, attrs[p] ]);
// 	// 			delete attrs[p];
// 	// 		}
// 	// 	}
// 	// }
...
// 			transformHyperscript(root.children[i], handlers, key+'.'+i);
// 		}
// 	}
// }
