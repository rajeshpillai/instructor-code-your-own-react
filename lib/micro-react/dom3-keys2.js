...
var TinyReact = (function () { 
    ...
    const diff = function (newvDom, container, oldDom) {
        ...
        if (typeof newvDom.type === "function") {
            ...
            oldDom._virtualElement = newvDom;   // Verify as this is updating newvdom
            
            //Recursively diff children

            if (oldDom.childNodes.length >= newvDom.children.length) {
                console.log("Length: old = new ", oldDom.childNodes.length, newvDom.children.length);
                console.log("compare: ", oldDom.childNodes, newvDom);
            }
            ...
            if (Object.keys(keyedElements).length === 0) {
                ...
                newvDom.children.forEach((virtualElement, i) => {
                    ...
                    if (key) {
                        ...
                        if (keyedDomElement) {
                            diff(virtualElement, oldDom, keyedDomElement);
                            keyedDomElement._dirty = true;
                        }
                        else {
                            // Mount: new element
                            mountElement(virtualElement, oldDom, keyedDomElement);
                        }
                    }
                });
            ...
            if (Object.keys(keyedElements).length === 0) {
                ...
                if (oldNodes.length > newvDom.children.length) {
                    for (let i = 0; i < oldDom.childNodes.length; i += 1) {
                        ...
                        for (let n = 0; n < newvDom.children.length; n += 1) {
                            if (newvDom.children[n].props.key == oldKey) {   // no need to type check
                                found = true;
                                break;
                            }
                        }
