...
// 12. Added replace child method (for optimization)
// 13. Old is native and new is component and vice versa
...
var TinyReact = (function() {
  const createElement = function(type, attributes = {}, ...children) {
    ...
    let props = Object.assign({ children: childElements }, attributes);
    if (Object.freeze) {
      Object.freeze(props);
    }
    return {
  ...
  const diff = function(vdom, container, oldDom) {
    ...
    if (!oldDom) {   // If no old dom, may be first time rendering
      mountElement(vdom, container, oldDom);
    }
    else if (vdom.type !== oldvdom.type && (typeof vdom.type !== "function")) {
      // Replace dom, as  types have changed.
  ...
  function createDomElement(vdom, container, oldDomElement) {
    let newDomElement = null;
    if (typeof vdom.type === "function") {
      if (TinyReact.Component.isPrototypeOf(vdom.type)) {
        let instance = null;
        if (vdom.component) {
          instance = vdom.component;
        } else {
          instance = new vdom.type(vdom.props);
          if (instance.props.ref) {
            instance.props.ref(instance);
          }
        }
        let compvDom = instance.render();
        compvDom.component = instance;
        newDomElement = createDomElement(compvDom, container, oldDomElement);
      } else {
  ...
  function getKey(vdom) {
    ...
    return component ? component.props.key : vdom.props.key;
  }
  ...
  function updateComponent(newVirtualElement,oldComponent,container,domElement) {
    ...
    if (oldComponent.shouldComponentUpdate(newVirtualElement.props)) {
      ...
      oldComponent.props = newVirtualElement.props;
  ...
  function renderComponent(component, container, dom) {
    var p = component.nextProps,
      s = component.state;
    
    if (component.base) {
			if (component.shouldComponentUpdate(p,s) === false) {
				component.props = p;
				return;
			}
      component.componentWillUpdate(p, s);
    }
    
    component.props = p;
    if (component.nextState) {
      component.updateState(component.nextState);
    }
    let rendered = component.render(p, s);
    rendered.component = component;
    
    //if (component.base) component.componentDidMount();
    
    diff(rendered, container, dom);
  }
  ...
  function renderFunctional(vdom) {
    const nextElement = vdom.type(vdom.props);
    return nextElement;
  }

  function renderStateful(vdom) {
    const component = new vdom.type(vdom.props);
    // Invoke LifeCycle
    component.componentWillMount();
    const nextElement = component.render();
    nextElement.component = component;
    component.base = nextElement;  // Assign the dom to component
  ...
  class Component {
    ...
    // todo: FIX THIS, store in nextState
    setState(newState, callback) {
      if (!this.prevState) this.prevState = this.state;
      
      // this.nextState = this._extend(
      //   this._extend({}, this.state),
      //   typeof newState === "function" ? newState(this.nextState, this.props) : newState
      // );

      this.nextState = Object.assign({}, this.state || {}, newState);
      ...
      renderComponent(this, container, dom);
    }

    // todo: do not setstate immediately as componentWillUpdate should file before
    // state changes.
